...
class App:
    def __init__(self):
        ...
        self.fruit = [(i * 60, randint(0, 104), randint(0, 3), True) for i in range(4)]
    ...
    def update(self):
        ...
        for i, v in enumerate(self.floor):
            self.floor[i] = self.update_floor(*v) # *v is pointer?
    ...
    def update_player(self):
        ...
        # 더블점프
        if pyxel.btn(pyxel.KEY_SPACE) and self.double_jump > 0:
            self.player_vy = -5
            self.double_jump -= 1
    ...
    def draw(self):
        ...
        # 점수
        s = "SCORE {:>4}, Double Jump {:>4}".format(self.score, self.double_jump)
        pyxel.text(5, 4, s, 1)
        pyxel.text(4, 4, s, 7)
